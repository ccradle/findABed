# pii-disclosure-public Specification

## Purpose

Public-facing surfaces (root `index.html`, audience FOR-*.md docs, government adoption guide, hospital privacy summary, AsyncAPI contract, right-to-be-forgotten doc) accurately and honestly disclose the platform's PII handling posture. Claims SHALL be path-scoped (DV referral path vs navigator-hold path) so that visitors, CoC admins, city officials, hospital privacy officers, and funders are not misled by a platform-wide assertion that no longer holds in v0.55+.
