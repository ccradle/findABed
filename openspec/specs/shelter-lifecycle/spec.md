...
Shelter activation/deactivation lifecycle. Covers admin deactivation with reason tracking, hold cascade, DV safety gate, coordinator visibility of inactive shelters, reactivation, and audit trail. V52 added the `active` boolean + bed search filter. V53 added deactivation metadata. V54 added `CANCELLED_SHELTER_DEACTIVATED` reservation status for the cascade.
...
- **AND** inactive shelters are visually distinct (muted background, preserved text contrast — no opacity-based degradation)
...
- **AND** for DV shelters, the dialog includes an additional safety warning

#### Scenario: Admin toggles shelter to active
- **WHEN** an admin clicks the reactivate action on an inactive shelter row
- **THEN** a confirmation dialog appears with a message about the coordinator needing to update availability
...
- **THEN** the shelter appears in their dashboard with a muted background and an "Inactive" badge
- **AND** bed update controls (stepper, availability form) are disabled via `aria-disabled="true"` and `pointer-events: none`
- **AND** a message displays: "This shelter was deactivated on {date}. Contact your CoC admin to reactivate."
