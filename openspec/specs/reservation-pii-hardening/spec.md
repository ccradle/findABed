# reservation-pii-hardening Specification

## Purpose

Hardens the v0.55 navigator-hold opt-in PII path with end-to-end forensic and operational evidence: structured audit events for write/read/purge lifecycle, generic exception messages, a 15-minute purge cadence floor, bounded UPDATE statements, list-view PII suppression, no-store cache headers, and OpenAPI annotations. The capability ensures the public 24-hour purge claim is operationally verifiable, that PII does not leak via list responses, exception messages, log lines, browser caches, or external schemas.
