...
Structured per-shelter eligibility criteria — including criminal record acceptance policy, program requirements, documentation, intake hours, and custom tags — used by bed search to filter shelters by reentry-relevant criteria, and surfaced with a non-dismissable disclaimer reminding outreach workers that criminal record policies are self-reported and must be verified directly with the shelter before assuming eligibility.
