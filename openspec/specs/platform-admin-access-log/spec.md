# Capability: platform-admin-access-log

## Purpose
Tamper-evident, append-only audit log of every `@PlatformAdminOnly` invocation. Every platform-operator action writes paired rows to `platform_admin_access_log` (operator-asserted justification) and `audit_events` (chained per Phase G-1 hash chain), enabling forensic reconstruction of who did what to which tenant and why.
