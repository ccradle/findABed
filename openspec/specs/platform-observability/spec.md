...
Platform-wide (instance-level, tenant-agnostic) observability configuration: Prometheus metrics on/off, OpenTelemetry tracing on/off + endpoint, and the three monitor cadences (stale-shelter detection, DV canary probe, NOAA temperature fetch). All reads happen at JVM/scheduler runtime; writes are PLATFORM_OPERATOR + `@PlatformAdminOnly` + `X-Platform-Justification` header. Storage is a single-row `platform_config` table seeded with V98 migration. Replaces the broken-since-G-4.4 per-tenant `/admin#observability` flow with a single platform-tier surface on the Platform Operator Dashboard.
...
- **THEN** 400 with top-level `error: "missing_justification"` (handed
  by the `JustificationValidationFilter`; this filter writes the
  response directly, NOT through the `StructuredErrorException →
  context.errorCode` path other v0.56 errors use; v0.57 follow-up
  tracks the contract uniformity fix)
