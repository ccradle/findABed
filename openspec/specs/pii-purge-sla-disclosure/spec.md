# pii-purge-sla-disclosure Specification

## Purpose

Hold-attribution PII collected via the v0.55 navigator-hold opt-in path carries a 24-hour purge SLA. Because public-facing copy makes this a contractual claim, the platform's compliance posture matrix, per-version runbook, and CHANGELOG SHALL document the columns, encryption envelope, purge cadence, and operational evidence (or honest disclosure where the operational evidence is not yet wired). This capability ensures the public claim is not stronger than the operational verification an operator can perform.
