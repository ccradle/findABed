...
Observability test coverage: unit tests (SimpleMeterRegistry), integration tests (@AutoConfigureObservability), Karate @observability BDD features, dev tooling (dev-start.sh --observability), and admin UI for runtime config.
...
- **AND** the backend starts with `management.server.port=9091` for unauthenticated Prometheus scraping
...
- **THEN** Prometheus can scrape `host.docker.internal:9091/actuator/prometheus` without authentication
