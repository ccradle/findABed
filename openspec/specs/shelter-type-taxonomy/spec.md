...
Controlled-vocabulary classification of each shelter record (`shelter_type`) for search filtering and display. Coexists with the load-bearing `dvShelter` boolean (which remains the primary control for DV access and RLS); `shelter_type` is a self-reported descriptor with no implied compliance status. Supports reentry transitional housing search alongside emergency, DV, and treatment-facility classifications, with EN/ES localized labels.
