...
Tenant-level boolean flag (`tenant.config.dv_policy_enabled`) acknowledging that a Continuum of Care operates Domestic Violence shelters. The flag gates per-shelter `dv_shelter=true` writes via a `ShelterService` invariant (see also `shelter-edit` capability), is flipped via a dedicated COC_ADMIN PATCH endpoint with extra-confirm modal, and is forbidden from disable while active DV shelters exist on the tenant. Provides the tenant-level structural acknowledgement that all existing DV protections (VAWA-aware redaction, RLS, audit suppression, dvAccess JWT claim) presuppose. Backfilled to `true` on tenants with existing DV shelters via a Flyway migration on first deploy of v0.56.
...
- **WHEN** the V97 migration runs on a tenant with one or more shelters where `dv_shelter = true`
...
- **WHEN** the V97 migration runs on a tenant with no shelters where `dv_shelter = true`
...
- **WHEN** V97 has already run and the migration is re-applied
