...
## Purpose
Production-readiness hardening of the API key lifecycle, webhook subscription management, broadcast retry behavior, and audit logging discovered during pen-test review (#51) and the subsequent v0.32.2 verification. Created by syncing change `platform-hardening`. Update Purpose after archive.

The capability covers the security and reliability boundaries that Marcus Webb (pen tester), Alex Chen (principal engineer), and Marcus Okafor (CoC admin) flagged as production-blocking before pilot. SSE backpressure work was originally part of this spec but moved to `sse-backpressure-phase2` (#97) on 2026-04-10 because it warranted its own design and test plan.
