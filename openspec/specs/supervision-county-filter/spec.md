...
Per-shelter `county` field plus a bed search filter to support supervision-geography compliance for people on post-release supervision, who are restricted to an approved jurisdiction by their supervision order. County boundaries are jurisdictional (where a supervising officer has authority), not distance-based. Backed by per-tenant `active_counties` (default: NC 100-county list) administered by PLATFORM_OPERATOR; presented in OutreachSearch with desktop and mobile-collapsed UI variants.
