# Capability: platform-operator-ui

## Purpose
Browser-based UI for FABT platform operators (the staff identity defined in `platform-operator-identity`). The SPA exposes a `/platform/*` route tree — a lazy-loaded chunk separate from the tenant-admin bundle, gated by the build-time flag `VITE_PLATFORM_UI_ENABLED` so prod builds with the flag false dead-code-eliminate the chunk. Routes cover sign-in, MFA enrollment (QR + manual secret + 10 backup codes with print/copy confirmation modals), MFA verify, and a dashboard that fetches `GET /api/v1/auth/platform/me` and renders action cards grouped by category (Tenant Lifecycle, Operator Management, System Status). A persistent "PLATFORM OPERATOR MODE" banner with masked email, JWT-expiry countdown, and Logout button renders across every `/platform/*` route. JWTs live in sessionStorage (not localStorage, not cookies) and a `<PlatformProtectedRoute>` guard validates `iss=fabt-platform`, `mfaVerified=true`, JWT shape, and synchronous `exp` before any child fetch. All routes inherit the production CSP and pass an axe-core sweep with zero serious or critical violations.
