...
**Decision (revised 2026-04-24 — warroom pass-2 Marcus):** `held_for_client_name_encrypted` TEXT, `held_for_client_dob_encrypted` TEXT, `hold_notes_encrypted` TEXT — all nullable, all storing the v1 `EncryptionEnvelope` as base64. Fields are encrypted via `SecretEncryptionService.encryptForTenant(tenantId, KeyPurpose.RESERVATION_PII, plaintext)` on write and decrypted on read. The V87 migration bundles the `tenant_dek.purpose` CHECK-constraint update (add `RESERVATION_PII` to the allowed set) alongside the `ALTER TABLE reservation` column adds. See design Open Question #5 resolution + issue #152.
