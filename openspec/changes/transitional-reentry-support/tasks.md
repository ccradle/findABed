...
- [ ] 5.2 Bed search response DTO: include `shelterType`, `county`, `eligibilityCriteria` (full object), `requiresVerificationCall`, `acceptsFelonies` (derived from JSONB for convenience) — **DEFERRED**: filter behavior verified at the DB layer via inline readback in §13.4 tests; full DTO surface for read-side rendering can land alongside slice 4 frontend search-results work.
- [ ] 5.3 Shelter detail response DTO (`GET /api/v1/shelters/{id}`): include all new fields — **DEFERRED**: same rationale as 5.2; co-locate with slice 4 frontend shelter-detail UI.
