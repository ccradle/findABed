...
- **Flyway HWM**: V84 → V88 (4 migrations; renumbered post-v0.51.0 Phase F which consumed V79–V84). Verify actual HWM at implementation start in case Phase G claims additional slots.
...
**Open decision blocking V87 design** (originally V81 — `held_for_client_*` PII columns):
should these fields ride the `tenant_dek`-wrapped DEK infrastructure shipped in v0.51.0 Phase F-6, or remain plaintext with the 24h purge as originally designed? Marcus Webb flagged that shipping unencrypted PII columns two weeks after announcing per-tenant wrapped DEKs would be a regression against the v0.51.0 security posture. Tracked in **[GitHub issue #152](https://github.com/ccradle/finding-a-bed-tonight/issues/152)**.
