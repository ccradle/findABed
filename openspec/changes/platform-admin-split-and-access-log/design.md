...
- **F4 — Multi-module split for `fabt-cli`.** Per Decision 8, the CLI was specified as a separate Maven module producing `fabt-cli.jar`. During G-4.2 implementation we discovered the project is single-module (one `pom.xml` under `backend/`), and converting to multi-module touches CI scripts, Docker build paths, IDE configs, and dev-start.sh — all for a CLI that runs once-ever per platform_user activation. **G-4.2 deviation:** ship `org.fabt.tooling.HashPasswordCli` as a standalone class inside the backend artifact (relocated from the original `org.fabt.cli` package by the G-4.4 post-merge hotfix in commit `9067d53`; the stale `cli/` copy was removed during G-4.6 §5.10 dry-run cleanup). Operator invocation: `java -cp /app/app.jar -Dloader.main=org.fabt.tooling.HashPasswordCli org.springframework.boot.loader.launch.PropertiesLauncher` (or `htpasswd -bnBC 12` as a Spring-Security-compatible alternative). The original "separate JAR artifact" goal is preserved as F4: when ops volume justifies (e.g., a non-VM deployment target needs the CLI but not the backend), split via `maven-shade-plugin` execution producing `fabt-cli-${version}.jar` with `org.fabt.tooling.HashPasswordCli` as the main class — does NOT require multi-module restructure.
...
- **F12 — TenantLifecycleController REST endpoints (decided 2026-04-25, shipped as G-4.6 in v0.53 on 2026-04-26). ✅ STATUS: shipped.** During G-4.4 endpoint inventory we discovered that tasks.md §5.3 referenced a `TenantLifecycleController` (suspend / unsuspend / offboard / hardDelete) that did not actually exist — Phase F shipped `TenantLifecycleService` + the state machine but never landed REST endpoints. Without this slice, tenant lifecycle actions in production happen only via `psql` against the DB owner: no audit row, no MFA gate, no operator identity, no justification. **Decision: add G-4.6 to v0.53.** Cost was ~6h actual (controller + IT + warroom round). Value is completing the audit-chain story end-to-end so every tenant lifecycle action gets its PAL + chained AE row with operator identity + justification. **Implementation:** 4 endpoints in `TenantLifecycleController.java` carrying `@PreAuthorize("PLATFORM_OPERATOR") + @PlatformAdminOnly` + state-capture allowlist `{slug, name, state, archivedAt}`; new `TenantLifecycleExceptionAdvice` with `@Order(HIGHEST_PRECEDENCE)` mapping FSM rejections → 409. Gated behind `fabt.tenant.lifecycle.enabled` (default false); flag stays off in prod until operators rehearse against staging. **Warroom-driven follow-ups (F31-F34) captured below.** Out-of-band psql lifecycle actions remain documented in the crypto-shred runbook as the supported path until the flag flips, then as emergency-only (and break the audit posture on purpose).
...
- **F23 — Existing bucket4j entries collapse to nginx's IP behind the proxy (Marcus/Sam HIGH, pre-existing bug surfaced during G-4.5 §6.b warroom).** Every bucket4j cache entry in `application.yml` (`rate-limit-login`, `rate-limit-platform-login`, `rate-limit-password-change`, `rate-limit-admin-reset`, `rate-limit-forgot-password`, `rate-limit-verify-totp`) uses `cache-key: getRemoteAddr()`. In production behind nginx, `request.getRemoteAddr()` returns nginx's container IP — every distinct client maps to ONE bucket, neutralizing the per-IP throttle for the highest-risk endpoints (login, password change, MFA verify). `ApiKeyAuthenticationFilter` has the correct pattern (private `resolveClientIp` reads X-Real-IP first); G-4.5 §6.b §6.7 lifted that into a `ClientIpResolver` shared utility AND uses a SpEL cache-key in its new `rate-limit-dv-referral-create` entry. **Fix path:** sweep the 6 pre-existing entries to use the same SpEL pattern (`getHeader('X-Real-IP') != null and !getHeader('X-Real-IP').isBlank() ? getHeader('X-Real-IP') : getRemoteAddr()`) AND refactor `ApiKeyAuthenticationFilter.resolveClientIp` to delegate to `ClientIpResolver.resolve()` so all rate-limit / abuse-detection code resolves identically. ~30 min slice. Worth landing before any production tenant is exposed to internet brute-force traffic; not a v0.53 blocker because the demo deployment's audience is small and the threat model differs.

- **F24 — `referral_token` partial index doesn't match `deleteStalePendingForTenant` predicate (Sam MEDIUM).** V41 partial index is `(tenant_id, expires_at) WHERE status='PENDING'`. The new demo-cleanup DELETE filters by `(tenant_id, status, created_at)` — the planner uses the partial index for the `(tenant_id, status)` prefix and then heap-scans by `created_at`. Microseconds for demo data (≤a few hundred PENDING rows per tenant); becomes meaningful if a tenant ever sustains a deeper PENDING backlog. **Fix path:** add a Flyway migration creating `(tenant_id, created_at) WHERE status='PENDING'` if a multi-tenant non-demo deployment ever runs this cleanup pattern, OR add a separate index variant for the demo cleanup query. ~30 min slice; defer until a non-demo tenant has the cleanup enabled.

- **F25 — Demo cleanup cron is hardcoded in @EventListener (Riley/Alex MEDIUM, pattern shared with referralEscalation).** `DvReferralDemoCleanupJobConfig.registerWithScheduler` calls `batchJobScheduler.registerJob(..., "0 0 */6 * * *", true)` directly — not driven by `tenant_config.batch_schedules` JSONB. The existing `ReferralEscalationJobConfig` uses the same hardcoded pattern (`"0 */5 * * * *"`); both jobs CAN be retuned at runtime via `BatchJobScheduler.rescheduleJob` (already exposed) but neither participates in the `BatchJobScheduler.DEFAULT_SCHEDULES` map. **Fix path:** when an operator actually needs to retune cadence without a restart (e.g. demo audience expands and a 4h cycle is preferred over 6h), move both jobs to the JSONB-driven pattern by adding entries to DEFAULT_SCHEDULES + reading tenant config in registerWithScheduler. ~1h slice spanning both jobs to keep them consistent.

- **F26 — TOTP secret copy-to-clipboard ships sensitive material with no clear timer (Marcus MEDIUM, surfaced during G-4.5 §6.c warroom).** The new "Copy" button next to the manual-entry secret on TotpEnrollmentPage writes the TOTP seed to `navigator.clipboard`. iOS shows clipboard contents to other apps for ~60 seconds; Universal Clipboard syncs across Apple devices; many password managers monitor clipboard contents. The seed is the long-lived shared secret for TOTP — anyone with momentary clipboard access can clone the user's authenticator. **Fix paths:** (a) brief toast "Secret copied — paste into your authenticator now" so the user knows to act fast and not leave the secret on the clipboard; (b) 30-second auto-clear via `setTimeout(() => navigator.clipboard.writeText(''), 30000)` (with caveats — some browsers block secondary writes outside user-gesture contexts); (c) document the trade-off in the totp.copySecret tooltip and rely on user awareness. Recommend (a)+(c) for minimum effort, (b) added before any production tenant ships platform-operator MFA. ~1h slice including i18n strings + Toast component if not already present.

- **F27 — Print template lacks a "retrieve from printer immediately" warning banner (Casey MEDIUM, surfaced during G-4.5 §6.c warroom).** The new print stylesheet on the backup-codes page renders codes prominently (18pt monospace, black on white) but does not include a header warning. A user who prints the page in a public location (library, work printer queue, shared home printer with multiple users) leaves a credential leak — physical access to the printout = full account-recovery capability. The "Save as PDF" path through the same print dialog has the same persistence concern but on disk instead of paper. **Fix path:** add a print-only warning banner via a `.fabt-print-only` class with `display: none` in normal viewport + `display: block` in `@media print`. Banner text: "Sign-in verification backup codes. If printed in a public location, retrieve from the printer immediately. Anyone with access to this paper can recover your account." ~30 min slice including i18n string. Worth landing before any production tenant onboards platform operators who would print this page.

- **F28 — `FabtPlatformUserDelayedActivation` alert + backing gauge deferred (Jordan MEDIUM, scoped out of G-4.5 §6.d).** Spec'd alert needs a gauge tracking count of `platform_user` rows where `created_at > 24h ago AND mfa_enabled = false` so an operator who forgot to activate a freshly-created platform user gets paged. Implementation requires (a) new V94 SECURITY DEFINER function `platform_user_count_unactivated_older_than(threshold_seconds INT) RETURNS INT` (V87 REVOKEd direct table access from fabt_app, so all reads go through SECURITY DEFINER), (b) a `PlatformUserActivationGauge` component with `@Scheduled(fixedRate = 60_000)` refresher updating an AtomicInteger gauge `fabt.platform.user.unactivated.count`, (c) the alert rule itself in `phase-g-platform-admin.rules.yml`. Deferred because: (i) the V94 migration is the heaviest piece and warrants its own slice, (ii) the operational gap is partially covered by `PlatformUser.isLoginAllowed()` (an unactivated user CANNOT log in even if forgotten) plus the §5.10 runbook activation checklist. **Fix path:** ship as a follow-up commit after v0.53 deploys; ~1h slice spanning the migration + gauge + rule.

- **F29 — Should defense-in-depth aspect rejections emit a synthetic audit row? (Marcus/Casey debate, surfaced during G-4.5 §6.d warroom).** The new aspect-side `X-Platform-Justification` check at `PlatformAdminLogger.aroundPlatformAdminOnly:114` (and the existing MFA_VERIFIED check at line 116) reject with `AccessDeniedException` BEFORE `commitAuditRows`. Result: rejected requests produce metric + WARN log + MDC marker but NO `audit_event` row. **Marcus's view:** consistent with the MFA_VERIFIED gate; the metric (tagged by action) + filterable logs are the forensic surface; the alert itself is the audit equivalent for "should never happen" bugs. **Casey's view:** the audit chain is the load-bearing forensic record; ANY rejected platform action — even a defense-in-depth catch — should land in audit_events with a synthetic `AUDIT_VIOLATION_REJECTED` action so the chain is complete. **Fix path if Casey wins the debate:** add an AUDIT_VIOLATION_REJECTED enum value to AuditEventType, write a row from the aspect's catch arm BEFORE throwing, audit-test the new path. ~1h slice. **Recommendation:** defer until Casey explicitly weighs in; current parity-with-MFA_VERIFIED-gate is the right principle if no compliance pressure says otherwise.

- **F30 — `FabtPlatformLoginFailureBurst` threshold may miss slow-drip distributed enumeration (Jordan MEDIUM, surfaced during G-4.5 §6.d warroom).** Threshold of `>5/min sustained 2min` catches distributed bursts; bucket4j catches single-IP brute force; nothing catches slow-drip distributed enumeration (1 attempt every 90s for hours from rotating IPs). Calibration data isn't available yet — depends on real legitimate platform-operator activity baseline post-deploy. **Fix path:** add a SECOND alert at `>1/min over 30min` with severity=warning + matching annotation describing the slow-drip threat model. ~30 min slice. **Recommendation:** defer to first-deployment calibration window; not worth speculative tuning.

- **F31 — `@PlatformAdminOnly` `tenantId`-param convention should be ArchUnit-pinned (Jordan HIGH, surfaced during G-4.6 warroom).** `PlatformAdminLogger.resolveActionTenantId` only matches a method parameter named exactly `tenantId` of type UUID. A controller author who names their `@PathVariable` `id` (URL-friendly) silently lands the per-tenant audit-event row in the SYSTEM chain instead of the target tenant chain — root cause of the G-4.6 IT failures #2 and #3 before fix. **Already shipped in v0.53 (G-4.6 warroom HIGH):** runtime WARN + `fabt.platform.audit.tenant_id_fallback{action=...}` Counter at `PlatformAdminLogger.resolveAndWarnOnFallback` so the next regression surfaces on a Grafana panel and in logs within seconds. The Counter excludes `PLATFORM_TENANT_HARD_DELETED` (legitimately SYSTEM-scoped per Decision 13) and platform-owned non-tenant actions (`PLATFORM_BATCH_JOB_TRIGGERED`, `PLATFORM_USER_*`). **Deferred to v0.54+:** an ArchUnit rule pinning the convention at compile/test time — every method annotated `@PlatformAdminOnly` whose `emits` is a tenant-scoped `PLATFORM_TENANT_*` (non-HARD_DELETED) value MUST have a parameter named exactly `tenantId` of type `UUID`. Cleaner long-term: a `@TenantId` meta-annotation that the aspect reads instead of the parameter-name string match (decouples the audit chain from method-signature conventions, but requires aspect changes + migration of existing call sites). ArchUnit alone is the cheaper v0.54 win. ~30 min slice for the rule + an ArchUnitTest covering the existing `@PlatformAdminOnly` controllers. The meta-annotation is a separate v0.55+ refactor decision.

- **F32 — IT coverage of state-capture allowlist requires an ArchUnit guard against reflection-based regression (Marcus LOW, G-4.6 warroom).** `TenantLifecycleController.captureLifecycleBefore`/`After` hand-pick four fields (`slug, name, state, archived_at`) — credentials, OAuth secrets, DEKs are unreachable. If a future refactor "simplifies" via `BeanUtils.describe(tenant)` or reflection, the allowlist evaporates silently and any sensitive field added to `Tenant` leaks into the PAL row. **Fix path:** ArchUnit rule that methods named `captureLifecycle*` (or methods within `@PlatformAdminOnly`-bearing controllers that call `PlatformActionStateCapture.capture*`) MUST NOT call `BeanUtils`, `BeanWrapperImpl`, `Class.getDeclaredFields`, or any reflection on `Tenant` / other entity types. ~30 min slice; bundle with F31's ArchUnit work for v0.54.

- **F33 — `TENANT_*_REJECTED` audit-row coverage IT is service-layer-only (Riley LOW, G-4.6 warroom follow-on).** `TenantLifecycleControllerTest.suspendAlreadySuspended_rejected_butPalPersists` now asserts `TENANT_SUSPEND_REJECTED >= 1` (G-4.6 warroom MEDIUM fix). The other three rejection paths (`TENANT_UNSUSPEND_REJECTED`, `TENANT_OFFBOARD_REJECTED`, `TENANT_HARD_DELETE_REJECTED`) are exercised by `TenantLifecycleServiceIntegrationTest` at the service layer but NOT through the controller IT. A regression where `wrapAttemptAudit` works for suspend but breaks for one of the other three (e.g., a copy-paste bug in a future refactor) would slip past green. **Fix path:** add three more rejection paths to the controller IT — `unsuspendAlreadyActive`, `offboardOnDeletedTenant`, `hardDeleteOnNonArchived` — each asserting both 409 + the matching `*_REJECTED` audit row count. ~45 min slice. Defer until the next G-4 lifecycle slice or post-deploy.

- **F34 — Push `fabt.tenant.lifecycle.enabled=true` into a shared test profile (Jordan LOW, G-4.6 warroom).** `TenantLifecycleControllerTest` and the existing `TenantLifecycleFeatureFlagEnabledIntegrationTest` both use `@SpringBootTest(properties = "fabt.tenant.lifecycle.enabled=true")`. Each `properties` override forks the Spring context cache — measurable test-suite startup tax that grows linearly as more lifecycle ITs land. **Fix path:** add `fabt.tenant.lifecycle.enabled=true` to a shared `application-lifecycle-test.yml` profile and switch the ITs to `@ActiveProfiles({"test","lifecycle-test"})`. Both ITs share the same context, ~5-10s saved per run as the suite grows. ~20 min slice; defer until a third lifecycle IT lands.

- **F35 — `oracle-update-notes-vX.Y.Z.md` §6.3 + §6.7 contracts drifted from current source (HIGH, surfaced during v0.53 deploy execution).** Three runbook §6 commands had bugs that a deploy operator copy-pasting verbatim would have hit at deploy day: (a) §6.3 documents `FABT_BASE_URL=...` but the playwright config at `e2e/playwright/deploy/playwright.config.ts:34` reads `process.env.BASE_URL` — the wrong env var was silently ignored (default of `https://findabed.org` happened to be what we wanted, so it appeared to work). (b) §6.3 specifies `--project=deploy-verify` but the only project defined is `chromium` — would have failed with "no project matching." (c) §6.7's `/v3/api-docs` grep test was added during the G-4.6 doc sweep but is broken on the deployed v0.53 frontend: nginx routes `/v3/*` to the SPA, returning HTML (`<` first byte) — grep for the API path returns 0 always, regardless of whether the bean is registered. **Fix path:** §6.3 should use `BASE_URL=` + drop `--project=...` + recommend a positional spec filter (e.g. `deploy/post-deploy-smoke.spec.ts`); §6.7 should use authenticated `curl -X POST` to the actual endpoint with the platform JWT — flag-OFF returns 404 (no controller bean), flag-ON returns a real 4xx (controller dispatches but rejects on bad UUID / missing setup). Need to ground-truth-audit every smoke command per `feedback_runbook_groundtruth_vm.md` before the next deploy. ~30 min slice for the runbook updates.

- **F36 — `app-tenant-name`/`app-tenant-name-footer` testIDs are `!isMobile`-gated; Playwright smoke fails on mobile-viewport branches (MEDIUM, surfaced during v0.53 §6.3 smoke).** `Layout.tsx:319` conditionally renders `data-testid="app-tenant-name"` only when `!isMobile`, and the footer twin at `Layout.tsx:632` mirrors that pattern. The `post-deploy-smoke.spec.ts` test 13 (Blue Ridge admin login → tenant-identity assertion) failed because Playwright's chromium hit the mobile branch — testID didn't render, `getByTestId('app-tenant-name')` couldn't find it. The page DID load correctly per the failure trace's snapshot (header chip showed "Blue Ridge CoC (demo)" via accessible-name) — this is a test/frontend-coupling issue, NOT a deploy regression. **Fix paths:** (a) Render the testID elements at all viewports (e.g. `display: none` mobile-side via CSS, but DOM presence preserved for tests + a11y), OR (b) Pin Playwright's viewport explicitly to a desktop size so the desktop branch consistently renders, OR (c) Add a separate mobile-viewport testID set + parallel test variants. Recommend (a) for the smallest blast radius; ~30 min slice. Same fix in `Layout.tsx:632` for the footer twin. Save as `feedback_testid_viewport_brittleness.md` so the lesson sticks for future testID work.
