...
- [ ] 4.4 Create `JustificationValidationFilter.java` (Spring filter, NOT aspect — Alex's split recommendation): checks `X-Platform-Justification` header presence + length >= 10 chars on requests to `@PlatformAdminOnly`-annotated endpoints; rejects 400 if invalid.
