...
- [ ] 1.1 Create `feature/dv-policy-tenant-flag` branch in the code repo (`finding-a-bed-tonight/`); push to origin to establish remote tracking.
- [ ] 1.2 Create `feature/dv-policy-tenant-flag` branch in the docs repo (`findABed/`); push to origin.
- [ ] 1.3 Confirm clean working tree on both repos; record starting HEADs in a session-scratch file.

## 2. Backend — domain + persistence

- [ ] 2.1 Add `Tenant.isDvPolicyEnabled()` helper in `org.fabt.tenant.domain.Tenant` reading the JSONB `dv_policy_enabled` key, defaulting to `false` on absent key. Cover the parsing path that matches `Tenant.getConfig()` plus a defensive try/catch on JsonProcessingException returning `false` (mirrors the conservative read pattern in `ReservationConfigController.readHoldDurationFromConfig`).
- [ ] 2.2 Unit-test `Tenant.isDvPolicyEnabled()` with three fixtures: absent key returns `false`; `true` literal returns `true`; `false` literal returns `false`. One additional fixture for a malformed JSON value returning `false`.
- [ ] 2.3 Add `TenantService.setDvPolicyEnabled(UUID tenantId, boolean value)` writing the JSONB key under `tenant.config.dv_policy_enabled`. Mirror the JSONB-merge approach in `TenantService.setHoldDurationMinutes`. Idempotent against the existing value.
- [ ] 2.4 Unit-test `TenantService.setDvPolicyEnabled` for round-trip on existing-config tenants (no other config keys clobbered) and on empty-config tenants (initial JSONB write).

## 3. Backend — Flyway backfill migration

- [ ] 3.0 **(Sam)** V-prefix collision check at apply time: list `db/migration/V*.sql` files and confirm the next-free V-prefix. Resume-point assumes V94 (reentry-spec landed V93), but if anything else slips into the release window first, bump the new file accordingly. Update §3.1 file name and any references in tasks/spec/design accordingly. Document the chosen V-number in the PR description.
- [ ] 3.1 Create `V<N>__backfill_dv_policy_enabled.sql` (N from §3.0). Migration body: `UPDATE tenant SET config = jsonb_set(COALESCE(config, '{}'::jsonb), '{dv_policy_enabled}', 'true'::jsonb, true) WHERE id IN (SELECT DISTINCT tenant_id FROM shelter WHERE dv_shelter = true);` Add a SQL comment noting Sam's performance analysis (existing index on `shelter(tenant_id)` makes the EXISTS subquery fast even at 10K-shelter scale).
- [ ] 3.2 Add a flyway IT scenario verifying that a tenant with one or more `dv_shelter=true` rows in seed data lands on `dv_policy_enabled = true` after migration. (Exists as scenario "Tenant with existing DV shelters backfills to true" in dv-policy-tenant-flag spec.)
- [ ] 3.3 Add a flyway IT scenario verifying that a tenant with zero `dv_shelter=true` shelters does NOT have a `dv_policy_enabled` key written. (Exists as scenario "Tenant with no DV shelters is not modified".)
- [ ] 3.4 Add an idempotency test re-applying the migration on already-backfilled state and asserting no drift. (Pattern: use a Flyway `repeat` or manually re-execute the SQL via `JdbcTemplate` in test.)
- [ ] 3.5 Update `seed-data.sql` to include `tenant.config.dv_policy_enabled = true` on the 3 dev/demo tenants (`dev-coc`, `dev-coc-east`, `dev-coc-west`) so a fresh `--fresh` reseed without running V94 first still produces a coherent state. (Defensive belt-and-suspenders: the migration runs on Spring Boot startup before app traffic, so seed-only paths still work, but redundancy keeps `--fresh` audits clean.)
- [ ] 3.6 **(Riley)** Multi-tenant migration IT fixture: assemble a test tenant set covering all four migration cases — (a) tenant with active DV shelters, (b) tenant with only inactive DV shelters, (c) tenant with non-DV shelters only, (d) tenant with zero shelters. Assert post-migration JSONB state for each. Tests MUST create their own tenants per `feedback_isolated_test_data` and not piggyback on `dev-coc*`.

## 4. Backend — DvPolicyController PATCH endpoint

- [ ] 4.1 Add DTO `DvPolicyRequest(@NotNull Boolean dvPolicyEnabled)` in `org.fabt.tenant.api`. Bean-validation: `@NotNull` only; the field's domain is the boolean itself, not a numeric range.
- [ ] 4.2 Add `DvPolicyController` in `org.fabt.tenant.api` mirroring `ReservationConfigController` shape. Endpoint: `PATCH /api/v1/admin/tenants/{tenantId}/dv-policy`. Annotations: `@PreAuthorize("hasRole('COC_ADMIN')")`, NOT `@PlatformAdminOnly`.
- [ ] 4.3 **(Marcus)** Implement tenant-scoping guard: throw `AccessDeniedException` when `TenantContext.getTenantId()` is null OR not equal to the path `tenantId`. (Identical pattern to `ReservationConfigController.updateHoldDuration`.) **The tenant-scoping check MUST execute BEFORE any DV-shelter inventory query so a probe from Tenant A cannot learn whether Tenant B has DV shelters via timing or via the error message.** A 403 from cross-tenant access carries no body (no existence-leak).
- [ ] 4.4 Implement read-current-value-before-write so the audit row carries `old_value` (mirrors hold-duration's `readHoldDurationFromConfig`).
- [ ] 4.5 **(Marcus, Casey)** Implement disable-path guard: when `dvPolicyEnabled == false` AND `Tenant.isDvPolicyEnabled() == true` (i.e. true → false transition), query `SELECT COUNT(*) FROM shelter WHERE tenant_id = ? AND dv_shelter = true AND active = true`. If count > 0:
...
- [ ] 4.6 Implement enable-path: when `dvPolicyEnabled == true` (false → true OR true → true), call `TenantService.setDvPolicyEnabled(tenantId, true)` unconditionally and emit `TENANT_CONFIG_UPDATED` with `outcome: "applied"`.
- [ ] 4.7 Implement allowed-disable-path: when `dvPolicyEnabled == false` AND no active DV shelters exist (or flag was already `false` — idempotent re-disable), call `TenantService.setDvPolicyEnabled(tenantId, false)` and emit `TENANT_CONFIG_UPDATED` with `outcome: "applied"`.
- [ ] 4.8 **(Marcus, Riley)** Add controller IT covering all 11 scenarios in the dv-policy-tenant-flag spec under "PATCH dv-policy endpoint", "Enable path", "Disable path", and "Audit on flag change and on rejected disable". Use `@SpringBootTest` + `MockMvc` + the existing `TestAuthHelper` and `setupSecondaryTenant` patterns. Specific assertions required:
...
- [ ] 4.9 **(Alex)** Update OpenAPI annotations (`@Operation`, `@Parameter`) so the swagger surface documents the new endpoint at the same depth as `ReservationConfigController`'s `updateHoldDuration` (full sentence summary, full description with link to the spec capability name, parameter description, response codes for 200 / 400 / 403). Not just `summary = "Update DV policy"`.
- [ ] 4.10 Update `DemoGuardFilter` per the §0.1 warroom decision: either allow the endpoint, or block it with a friendly message branch matching the `/api/v1/admin/tenants/[^/]+/dv-policy` pattern. Default to BLOCK on demo (mirrors hold-duration's demo posture) unless the warroom rules otherwise.
- [ ] 4.11 **(Alex)** Add a `// TODO(arch)` comment in `DvPolicyController` referencing a future ADR for JSONB-config-key endpoint convention. With this third instance (after `hold_duration_minutes` and arguably `features.reentryMode`), the next config endpoint should trigger an extraction.

## 5. Backend — ShelterService invariant guard

- [ ] 5.1 Add invariant check in `ShelterService.create`: if request sets `dv_shelter == true` AND `Tenant.isDvPolicyEnabled() == false`, throw a 400-mapping exception with structured error code `shelter.dvShelter.requiresDvPolicy` and message directing the operator to the admin Settings tab.
- [ ] 5.2 **(Tomás language tightening)** Add the same invariant check in `ShelterService.update`. The check fires when the *request* sets `dv_shelter == true` AND the *current persisted value* is `false` (the "flip-up" case) AND `Tenant.isDvPolicyEnabled() == false`. When the request and the current value are both `true` and the request is updating *other fields*, the invariant is a no-op (the existing row's `dv_shelter=true` predates this change OR was created under a previously-enabled flag — it stays untouched). When the request sets `dv_shelter == false`, the invariant is a no-op regardless of the tenant flag (flip-down is always allowed).
- [ ] 5.3 Add IT covering the scenarios in the modified `shelter-edit` spec ("Create rejected …", "Update rejected …", "Create allowed …", "Update allowed …", "Setting dvShelter=false succeeds …", "Existing dvShelter=true shelters unaffected …"). Reuse existing test fixtures.
- [ ] 5.4 **(Marcus, Alex — NEW)** Add the same invariant guard to `ShelterService.activate` (or whatever method handles re-activation of a deactivated shelter — confirm path at apply time). When re-activating a shelter where `dv_shelter == true` AND `Tenant.isDvPolicyEnabled() == false`, return 400 with `shelter.dvShelter.requiresDvPolicy`. Without this guard, an operator who deactivated DV shelters → disabled the flag → re-enabled the flag is fine, but an operator who deactivated DV shelters → disabled the flag → tries to re-activate without re-enabling could resurrect a DV shelter on a flag-off tenant. Add an IT scenario for this exact path.
- [ ] 5.5 **(Demetrius — NEW)** 211 CSV bulk-import path: when the importer ingests a CSV row where the source data implies `dv_shelter=true` AND the target tenant's `dv_policy_enabled == false`, the importer MUST reject *that row* with the structured error `shelter.dvShelter.requiresDvPolicy` while continuing to process other rows (per-row reject, not whole-batch fail). The existing import response surface (per-row results + summary) MUST surface these rejections to the operator. Locate the import code path during apply (search for `211` / `csv` / `BulkImport` / equivalent) and add the invariant call. If the import bypasses `ShelterService.create()` via a bulk JPA insert, route it through the service layer OR replicate the invariant inline (decide at apply time and document).
- [ ] 5.6 **(Demetrius — NEW)** IT for 211 CSV import with mixed DV / non-DV rows on a flag-off tenant: assert non-DV rows persist successfully, DV rows are rejected with the structured code, and the import summary includes per-row results.
- [ ] 5.7 Confirm no regression in the existing `dv-shelter-e2e-exclusion` canary suite (DV protections still operate purely on per-shelter `dv_shelter` flag — the invariant gates writes only).

## 6. Backend — error registry + audit details

- [ ] 6.1 Register `tenant.dvPolicy.cannotDisableWhileDvSheltersExist` and `shelter.dvShelter.requiresDvPolicy` in the project's structured-error-code registry (or whatever existing convention enumerates these — check `org.fabt.shared.errors` first).
...
- [ ] 6.3 **(Marcus — NEW)** Cross-tenant 403 attempts on this endpoint: emit a `TENANT_CONFIG_UPDATED` audit row (or a closely-related event type if one already exists for cross-tenant policy violations) on every cross-tenant probe. Details payload: `outcome: "rejected"`, `rejection_code: "tenant.crossTenantAccess"`, plus the actor's tenant ID and the target tenant ID. This is defense-in-depth against lateral-movement signals — a COC_ADMIN from Tenant A trying to flip Tenant B's flag is forensically interesting. Confirm the existing `AuditEventType` enum covers this; add a value if needed.
