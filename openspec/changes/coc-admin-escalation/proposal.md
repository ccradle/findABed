...
- New table `escalation_policy` (Flyway V40): append-only, versioned per tenant + event_type. Each PATCH from admin creates a new row with `version+1`; old rows are never deleted (audit trail).
- New column `referral_token.escalation_policy_id` (Flyway V41): foreign key to the policy snapshot active when the referral was created. The escalation batch job looks up the *frozen* policy for each referral, not the current one. Mid-day policy changes apply only to new referrals.
...
- **Backend:** 2 Flyway migrations (V40 escalation_policy, V41 referral_token columns), new repository + service + cache for escalation policy, refactored `ReferralEscalationJobConfig`, new admin endpoints (escalated queue, claim, release, reassign, policy GET/PATCH), 5 new audit event types, ~10 new integration tests. ArchUnit boundary: escalation_policy stays in `notification` module.
- **Frontend:** new admin tab (`DvEscalationsTab.tsx`), updated `CriticalNotificationBanner` with CTA, new types, ~5 Playwright scenarios. Responsive (table → card list on mobile). Policy editor desktop-only.
- **i18n:** ~25 new keys in en.json + es.json. Keisha Thompson reviews all user-facing copy before merge.
- **Documentation:** new `docs/architecture.md` (Mermaid), new `docs/business-processes/dv-referral-escalation.md` (Mermaid sequence + state machine + matrix), updated `docs/FOR-DEVELOPERS.md`.
- **Operations:** zero new infrastructure. No new services, no new external dependencies, no deployment changes.
- **Performance:** policy lookup is cached (Caffeine, 5-min TTL); the escalation batch job runs every 5 minutes and now joins on `escalation_policy` via the FK. ArchUnit + benchmark verifies p95 batch job duration unchanged.
- **Security:** new endpoints are `COC_ADMIN+` only. RLS unchanged (CoC admin already had `dvAccess=true` access to referral_token via the existing shelter RLS chain). Marcus Webb sign-off on the policy PATCH endpoint required (it's the new admin write surface).
