...
- **Performance:** policy lookup is cached (Caffeine, 5-min TTL programmatically invalidated on PATCH); the escalation batch job runs every 5 minutes with per-run local caches eliminating N+1 hits. The new admin queue endpoint uses the V41 partial index `idx_referral_token_pending_by_expiry (tenant_id, expires_at) WHERE status = 'PENDING'` exploited by the tenant-filtered SQL. The auto-release scheduler uses the V41 `idx_referral_token_active_claim` partial index via the explicit `claimed_by_admin_id IS NOT NULL` predicate.
