...
- [ ] T-0: Create branch `feature/coc-admin-escalation` in code repo (`finding-a-bed-tonight`) from main.
- [ ] T-1: Capture pre-change baseline. Run `mvn test -Dtest='ReferralEscalationIntegrationTest,NotificationServiceTest'` and tee to `logs/coc-escalation-baseline.log`. Run the existing Playwright `persistent-notifications.spec.ts` through nginx and tee to `logs/coc-escalation-playwright-baseline.log`. Save as the regression bar.

### Backend — Schema (Flyway)

- [ ] T-2: **V40 — `escalation_policy` table.** Append-only versioned per (tenant_id, event_type). Columns: `id UUID PK`, `tenant_id UUID FK NULL`, `event_type VARCHAR(64)`, `version INTEGER`, `thresholds JSONB`, `created_at TIMESTAMPTZ`, `created_by UUID FK`. UNIQUE constraint on (tenant_id, event_type, version). Index `idx_escalation_policy_current` on (tenant_id, event_type, version DESC). RLS: unrestricted SELECT for `fabt_app` (admin reads via tenant filter in service layer); INSERT requires service-layer authorization. (D7)
- [ ] T-3: **V40 seed — platform default policy.** Insert one row with `tenant_id = NULL`, `event_type = 'dv-referral'`, `version = 1`, `thresholds = [{at: PT1H, ACTION_REQUIRED, [COORDINATOR]}, {at: PT2H, CRITICAL, [COC_ADMIN]}, {at: PT3H30M, CRITICAL, [COORDINATOR, OUTREACH_WORKER]}, {at: PT4H, INFO, [OUTREACH_WORKER]}]`. Matches the current hardcoded values exactly so existing tenants see no behavior change. (D7)
- [ ] T-4: **V41 — `referral_token.escalation_policy_id` column** (nullable UUID FK). Backwards compatible: existing rows have NULL, batch job falls back to platform default policy (`SELECT id FROM escalation_policy WHERE tenant_id IS NULL AND event_type='dv-referral' ORDER BY version DESC LIMIT 1`). (D7)
- [ ] T-5: **V41 — `referral_token.claimed_by_admin_id` and `claim_expires_at` columns** (nullable). Index `idx_referral_token_claim_expires` on `claim_expires_at` partial WHERE NOT NULL for the auto-release scheduler. (D4)
- [ ] T-6: **V42 — Audit event types.** No schema change; audit_events.event_type is already VARCHAR. Add the 6 new constants to the application's `AuditEventType` enum: `DV_REFERRAL_CLAIMED`, `DV_REFERRAL_RELEASED`, `DV_REFERRAL_REASSIGNED`, `DV_REFERRAL_ADMIN_ACCEPTED`, `DV_REFERRAL_ADMIN_REJECTED`, `ESCALATION_POLICY_UPDATED`. (D8)
