...
- [ ] 4.9c Unit test: cross-tenant cache-poisoning regression (per ADR "Cached-value tenant verification") — write a value with `TenantContext=A`; read with `TenantContext=B`; expect `IllegalStateException` tagged `CROSS_TENANT_CACHE_READ` + `fabt.cache.get{result=cross_tenant_reject}` incremented + `audit_events` row emitted. Defends against wrong-tenant-context-on-write (the #1 cache-leak pattern per Redis Inc. Feb 2026 post-mortem survey).
