...
- [ ] 4.9g Integration test scaffolded in `TenantScopedCacheAuditRollbackIntegrationTest` (commit `c4e9541`) but NOT yet run — requires Testcontainers Postgres boot (~90s) + Phase B `tenant` table seed. Pending: operator runs `mvn test -Dtest=TenantScopedCacheAuditRollbackIntegrationTest` locally or via CI `backend-integration` job before task closes.
