...
- [ ] 8.6 Implement `AuditChainVerifier` — daily scheduled integrity check; re-computes chain; fails if drift; emits alert
  - [ ] 8.6a **`canonicalizeJson` canonicalizer for `details` payload.** G-1 writer hashes Jackson's insertion-order JSON; DB stores JSONB (PG canonical: sorted keys + `": "` whitespace separator). Verifier reads `details::text` — a different form than the write-time hash input. The canonicalizer MUST produce a form both (a) a writer can commit to BEFORE hashing, and (b) the verifier can reproduce from PG's `::text` output. Simplest shape: Jackson tree parse + `ORDER_MAP_ENTRIES_BY_KEYS` + compact output with PG-matching whitespace. Apply identically in `AuditChainHasher.canonicalJson` AND the verifier — single source of truth for the canonical form.
