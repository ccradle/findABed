...
- [ ] 4.1 Create `TenantScopedCacheService` in `org.fabt.shared.cache` (new Spring bean, NOT `@Primary`) wrapping `CacheService`. Contract per spec `tenant-scoped-cache-service` + `tenant-scoped-cache-value-verification` + `tenant-scoped-cache-invalidate-tenant` + `tenant-scoped-cache-observability` (design-c D-C-8 through D-C-13): (a) key prefix with `|` separator sourced from `TenantContext.getTenantId()`, tagged `TENANT_CONTEXT_UNBOUND` ISE on miss; (b) value stamp-and-verify via `TenantScopedValue<T>(UUID tenantId, T value)` envelope — on read mismatch throw `CROSS_TENANT_CACHE_READ` ISE and persist audit row via `AuditEventPersister` under `@Transactional(REQUIRES_NEW)` so audit survives caller rollback; (c) `invalidateTenant(UUID)` iterates the eager-seeded `registeredCacheNames` registry + calls new `CacheService.evictAllByPrefix`; emits `TENANT_CACHE_INVALIDATED` audit row (event-bus path, normal rollback semantics); idempotent across retries; (d) emit `fabt.cache.get{cache,tenant,result}` + `fabt.cache.put{cache,tenant}` Micrometer counters with result ∈ {hit, miss, cross_tenant_reject, malformed_entry}. Exception messages carry action tags only — never UUIDs or payload. Defensive `IllegalArgumentException` on null value at put (belt-and-suspenders with task 4.7 ArchUnit rule).
- [ ] 4.1a Extend `CacheService` interface with `long evictAllByPrefix(String cacheName, String prefix)` (per design-c D-C-12 + spec `cache-service-evict-all-by-prefix`). Implement in `CaffeineCacheService` via `cache.asMap().keySet()` filter; in `TieredCacheService` stub for L2 (L1 path uses keyset filter today; Redis L2 future implementation uses `SCAN MATCH <prefix>* COUNT 1000` + `UNLINK` per batch, NEVER `KEYS` or `DEL`).
- [ ] 4.1b Eager-seed `TenantScopedCacheService.registeredCacheNames` at `@PostConstruct` via `CacheNames.class.getFields()` reflection (per design-c D-C-8). Publish `fabt.cache.registered_cache_names` Micrometer gauge + INFO-level startup log naming each seeded cache. Fail-fast on empty seed.
- [ ] 4.1c Add `TENANT_CACHE_INVALIDATED` + `CROSS_TENANT_CACHE_READ` + `MALFORMED_CACHE_ENTRY` constants to `AuditEventTypes`. Add `AuditEventTypesTest` pins per existing convention (every constant non-null + non-empty).
- [ ] 4.8 Unit test: `TenantScopedCacheService` throws `IllegalStateException` tagged `TENANT_CONTEXT_UNBOUND` on both `get` and `put` when no `TenantContext` is bound. Exception message does NOT contain any UUID.
- [ ] 4.9 Unit test: key prefixing applied with `|` separator; tenant A put + tenant A get returns the written value; tenant B get for the same logical key returns cache miss (prefix defence); hit/miss Micrometer counters increment correctly.
- [ ] 4.9b Unit test: `invalidateTenant(tenantA)` evicts tenant A entries across all cache names; tenant B entries in the same caches survive; `audit_events` row written with action `TENANT_CACHE_INVALIDATED` + per-cache eviction counts in details.
- [ ] 4.9c Unit test: cross-tenant cache-poisoning regression (spec `tenant-scoped-cache-value-verification`). Write a `TenantScopedValue` envelope stamped with tenant A into the raw underlying cache at tenant B's prefix (simulates wrapper-bypass or context-drift write); read via wrapper with `TenantContext=B`; expect `IllegalStateException` tagged `CROSS_TENANT_CACHE_READ` + `fabt.cache.get{result=cross_tenant_reject}` incremented. Defends against the #1 2025-2026 cache-leak pattern per Redis Inc. Feb 2026 post-mortem survey.
- [ ] 4.9d Unit test: malformed entry defence (design-c D-C-13 downside guard). Write a non-`TenantScopedValue` payload directly via raw `CacheService` (simulates pre-migration raw writer); wrapper `get` throws `IllegalStateException` tagged `MALFORMED_CACHE_ENTRY`; `fabt.cache.get{result=malformed_entry}` increments.
- [ ] 4.9e Unit test: `invalidateTenant` idempotency (spec `tenant-scoped-cache-invalidate-tenant` idempotency scenario). Call twice in sequence; first call returns N > 0 evictions; second returns 0; neither throws; two audit rows emitted.
- [ ] 4.9f Unit test: post-restart empty-registry safety (design-c D-C-8). New wrapper instance + no puts yet + `invalidateTenant(UUID)` on a never-written tenant iterates the eager-seeded `CacheNames` constants, returns 0 evictions per cache, emits an audit row with zero counts, and does NOT throw NPE.
- [ ] 4.9g Integration test: cross-tenant-read audit survives caller rollback (design-c D-C-9). Trigger a wrapper-detected cross-tenant read inside a `@Transactional` method; assert the caller's transaction rolls back; assert the `audit_events` row persisted via `REQUIRES_NEW` remains committed after the caller's rollback.
- [ ] 4.9h Unit test: defensive null-value rejection. Call `wrapper.put(cacheName, key, null, ttl)` → expect `IllegalArgumentException` immediately; no cache write occurs.
