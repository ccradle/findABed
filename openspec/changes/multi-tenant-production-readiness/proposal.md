...
- **Demo-site multi-tenant validation** (two additional permanent tenants seeded on `findabed.org` alongside `dev-coc`: `dev-coc-west` — Blue Ridge CoC (demo) — and `dev-coc-east` — Pamlico Sound CoC (demo) — each with a full user/shelter/referral matrix. Tenant names deliberately fictional (no such HUD-registered CoCs) to avoid partnership-implication risk per Casey. Pooling works as a live public proof across three tenants, not just a design document)
...
- **M1. Two additional permanent tenants in seed data** — TWO new permanent tenants added to `infra/scripts/seed-data.sql` alongside the existing `dev-coc`:
  - **`dev-coc-west` — "Blue Ridge CoC (demo)"** — UUID `a0000000-0000-0000-0000-000000000002`. Fictional western-region seed content. "Blue Ridge" is not a registered HUD CoC name.
  - **`dev-coc-east` — "Pamlico Sound CoC (demo)"** — UUID `a0000000-0000-0000-0000-000000000003`. Fictional eastern-region seed content. "Pamlico Sound" is not a registered HUD CoC name.
  Each tenant's seed matrix mirrors `dev-coc` scope: 6 role users (PLATFORM_ADMIN, COC_ADMIN, COORDINATOR, OUTREACH_WORKER, DV_COORDINATOR, DV_OUTREACH), 3–5 shelters (at least one DV shelter so the DV-access cross-tenant boundary is exercisable), sample bed availability, 1 sample pending DV referral. Idempotent `INSERT ... ON CONFLICT DO UPDATE` pattern; safe to re-run. Credential conventions: `admin@blueridge.fabt.org` / `admin123` for west users, `admin@pamlico.fabt.org` / `admin123` for east users (shared password mirrors `dev-coc` for demo-visitor convenience).
- **M2. Branding clarity — fictional regional names + "(demo)" suffix** — per Casey (revised 2026-04-20): use fictional region-themed names that do NOT match any real HUD-registered CoC jurisdiction. Blue Ridge and Pamlico Sound are geographic features (not jurisdictions), so there is no real "Blue Ridge CoC" or "Pamlico Sound CoC" a visitor could mistake for a live pilot deployment. The `(demo)` suffix is retained as belt-and-suspenders. Displayed tenant names are `Blue Ridge CoC (demo)` and `Pamlico Sound CoC (demo)` in login UI, landing page, admin panel header, page title, and all training materials. Seed data uses demonstrably-fictional shelter names (e.g., "Example House North", "Example Family Center"), fictional addresses (obviously non-geocodable patterns), and persona-derived fake contact names in BOTH tenants.
...
- **M6. Multi-tenant demo walkthrough** — new doc `docs/training/multi-tenant-demo-walkthrough.md`: 3-minute scripted visitor walkthrough covering all three tenants ("log in as `dev-coc`, observe shelters; log out; log in as `dev-coc-west` / Blue Ridge CoC (demo), observe different shelters + different DV posture; attempt cross-tenant URL, observe educational 404; log out; log in as `dev-coc-east` / Pamlico Sound CoC (demo), observe third regional posture; attempt another cross-tenant URL, observe educational 404"). Screenshot bundle folded into the `#120` pilot-readiness bundle. Linked from findabed.org landing page and FOR-COORDINATORS / FOR-COC-ADMINS audience docs.
...
- **M8. Seed migration safety gate (covers BOTH new-tenant migrations)** — BOTH Flyway migrations that create new demo tenants are reviewed pre-merge: **V76** (`dev-coc-west` / Blue Ridge) and **V77** (`dev-coc-east` / Pamlico Sound). Reviewers per migration: Casey (fictional-name + `(demo)` suffix enforcement + HUD-CoC-registry cross-check confirming no name collision), Marcus (no real-PII patterns), Maria (procurement-audience language). Review gate applies independently to each PR. Deploy sequence: both migrations land in prod → seed populates → M5 all-tenant post-deploy smoke green → only then does `opsx:archive` gate close.
...
- **V76 — `dev-coc-west` / Blue Ridge CoC (demo) tenant seed** (M1) — idempotent `INSERT ... ON CONFLICT DO UPDATE` for tenant row + 6 users + 3–5 shelters (at least one DV) + sample availability + 1 sample pending DV referral. UUID pinned at `a0000000-0000-0000-0000-000000000002`. Users under `@blueridge.fabt.org` email domain. Seed reviewed pre-merge by Casey / Marcus / Maria per M8. (V75 left unused as a breathing-room slot.)
- **V77 — `dev-coc-east` / Pamlico Sound CoC (demo) tenant seed** (M1) — idempotent `INSERT ... ON CONFLICT DO UPDATE` for tenant row + 6 users + 3–5 shelters (at least one DV) + sample availability + 1 sample pending DV referral. UUID pinned at `a0000000-0000-0000-0000-000000000003`. Users under `@pamlico.fabt.org` email domain. Seed reviewed pre-merge by Casey / Marcus / Maria per M8. Same review rigor as V76.
