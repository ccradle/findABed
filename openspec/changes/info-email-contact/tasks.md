...
- [x] 10.5 + 10.8 Authored `e2e/playwright/tests/contact-info-static.spec.ts` (5 tests, 22.6s, all green). Two test groups:
  ...
  - Mirrors `audience-pages-a11y.spec.ts`'s `FABT_DOCS_ROOT`-aware path resolution — CI without the docs repo checked out gets clean skips with descriptive reasons; operators with both repos cloned get the full run.
- [x] 10.6 Banned-words grep across the FULL info-email-contact diff (`git diff main..HEAD -- '*.html' '*.tsx' '*.ts' '*.java' '*.json' contact.js | grep '^+' | grep -v '^+++'`) for `24[ -]?hours?|reply within|respond within|we['']?ll get back|always reply|guarantee|every community` → 0 hits in added lines. Two pre-existing context-line hits (`This platform is not a guarantee...` in demo/index.html and demo/shelter-onboarding.html footer disclaimers) are anti-overclaim disclaimers that PREDATE this change; they remain unchanged.
- [x] 10.7 Real-name scan against `feedback_no_named_stakeholders_in_docs` (Sarah Dickerson + variants) on added lines → 0 hits. Per the memory: persona names (Marcus, Casey, Riley, Alex, Sam, Jordan, Devon, Maria) ARE allowed in committed docs and remain unrestricted; only real external-stakeholder names are restricted.
- [x] 10.8 Covered by §10.5 spec — see notes there. Both `lang="en"` and `lang="es"` branches verified.
