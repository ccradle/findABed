...
- [ ] T-29: Register new Micrometer metrics in `NotificationService` and `NotificationEmitter`:
  - `sse.queue.depth` — gauge per emitter (max across all emitters as a single gauge for simplicity, or histogram)
  - `sse.queue.drops{reason=critical_evicted|normal_dropped|coalesced}` — counter
  - `sse.disconnect.cause{reason=slow_client|forced_reconnect|shutdown|cap_evicted}` — counter
  - `sse.broadcast.rejected{eventType}` — counter
