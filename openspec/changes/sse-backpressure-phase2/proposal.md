...
- **`NotificationEmitter` wrapper class** that holds a bounded `LinkedBlockingQueue<OutboundEvent>` (capacity 32) and exposes only `enqueue(event)` — direct `send()` becomes a private implementation detail. This is the type-system enforcement of the single-writer invariant.
...
- **Forced periodic reconnect** every 25–30 minutes ± 3 minutes jitter (Netflix Zuul Push pattern). Prevents resource creep, state drift, and stale TCP connections. Client `EventSource` auto-reconnects and resends `Last-Event-ID`.
- **Graceful shutdown deadline**: `@PreDestroy` waits up to 5s for all per-emitter sender threads to drain and complete. Forces `complete()` after the deadline. JFR-tracked.
