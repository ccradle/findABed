...
- [x] 1.1 Initialize monorepo structure: `/backend`, `/frontend`, `/infra`, root `README.md`
- [x] 1.2 Initialize Spring Boot 3.4.x project in `/backend` with Java 21, Maven, groupId `org.fabt`, structured as a modular monolith
- [x] 1.3 Add core dependencies: Spring Web, Spring Security, Spring Data JDBC, PostgreSQL driver, Flyway, Caffeine, Jackson, Springdoc OpenAPI, Micrometer, Logback (JSON encoder), ArchUnit (test scope)
...
- [x] 2.1 Create Flyway baseline migration `V1__create_tenants.sql`: tenant table (id UUID PK, name, slug UNIQUE, config JSONB, created_at, updated_at)
- [x] 2.2 Create migration `V2__create_users.sql`: user table (id UUID PK, tenant_id FK, email, password_hash, display_name, roles TEXT[], dv_access BOOLEAN, created_at, updated_at)
- [x] 2.3 Create migration `V3__create_api_keys.sql`: api_key table (id UUID PK, tenant_id FK, shelter_id FK nullable, key_hash, key_suffix CHAR(4), label, role, active BOOLEAN, created_at, last_used_at)
- [x] 2.4 Create migration `V4__create_shelters.sql`: shelter table (id UUID PK, tenant_id FK, name, address_street, address_city, address_state, address_zip, phone, latitude, longitude, dv_shelter BOOLEAN, created_at, updated_at) with index on (tenant_id, dv_shelter)
- [x] 2.5 Create migration `V5__create_shelter_constraints.sql`: shelter_constraints table (shelter_id FK PK, sobriety_required, id_required, referral_required, pets_allowed, wheelchair_accessible, curfew_time, max_stay_days, population_types_served TEXT[])
- [x] 2.6 Create migration `V6__create_shelter_capacity.sql`: shelter_capacity table (shelter_id FK, population_type, beds_total) with composite PK (shelter_id, population_type)
- [x] 2.7 Create migration `V7__create_coordinator_assignments.sql`: coordinator_assignment table (user_id FK, shelter_id FK) with composite PK
- [x] 2.8 Create migration `V8__enable_rls.sql`: enable RLS on shelter table, create policy `dv_shelter_access` using `dv_shelter = false OR current_setting('app.dv_access', true)::boolean = true`
- [x] 2.9 Create migration `V9__create_import_log.sql`: import_log table (id UUID PK, tenant_id FK, import_type VARCHAR, filename, created_count, updated_count, skipped_count, error_count, errors JSONB, created_at)

## 3. Deployment Profiles

- [x] 3.1 Create `application.yml` with base configuration (server port, flyway, jackson, logging format)
- [x] 3.2 Create `application-lite.yml`: Caffeine cache config, disable Redis/Kafka auto-configuration
- [x] 3.3 Create `application-standard.yml`: Caffeine + Redis config, disable Kafka auto-configuration
- [x] 3.4 Create `application-full.yml`: Caffeine + Redis + Kafka config
- [x] 3.5 Implement `DeploymentTier` enum (LITE, STANDARD, FULL) and `DeploymentTierDetector` component
- [x] 3.6 Implement `CacheService` interface with `CaffeineCacheService` (lite) and `TieredCacheService` (standard/full) implementations using `@Profile`
- [x] 3.7 Implement `EventBus` interface with `SpringEventBus` (lite), `PgNotifyEventBus` (lite real-time), and `KafkaEventBus` (full) using `@Profile`
- [x] 3.8 Log deployment tier on startup; default to Lite with warning if no profile set

## 4. Multi-Tenancy and Roles

- [x] 4.1 Create `Role` enum: PLATFORM_ADMIN, COC_ADMIN, COORDINATOR, OUTREACH_WORKER (needed by all authorization checks in sections 4–7)
- [x] 4.2 Create `TenantContext` with ThreadLocal storage for current tenant ID
- [x] 4.3 Create `TenantFilter` servlet filter: extract tenant ID from JWT or API key, set on TenantContext, set `SET LOCAL app.dv_access` within a `@Transactional` boundary (service layer manages transactions, filter sets context before dispatch), clear on request completion
- [x] 4.4 Create `Tenant` entity and `TenantRepository` (Spring Data JDBC)
- [x] 4.5 Create `TenantService` with create, update, getById, getBySlug operations
- [x] 4.6 Create `TenantController` REST endpoints: POST/GET/PUT `/api/v1/tenants` with PLATFORM_ADMIN authorization
- [x] 4.7 Create `TenantConfigController`: GET/PUT `/api/v1/tenants/{id}/config` with COC_ADMIN authorization
- [x] 4.8 Add integration test: verify tenant isolation (user in tenant A cannot see tenant B data)

## 5. Authentication

- [x] 5.1 Create `User` entity and `UserRepository` (dvAccess defaults to false for new users — principle of least privilege)
- [x] 5.2 Create `JwtService`: generate access token (15min) and refresh token (7d) with userId, tenantId, roles[], dvAccess claims
- [x] 5.3 Create `AuthController`: POST `/api/v1/auth/login` (username/password → JWT), POST `/api/v1/auth/refresh` (refresh → new access token)
- [x] 5.4 Create `JwtAuthenticationFilter`: parse Bearer token, validate, set SecurityContext
- [x] 5.5 Create `ApiKey` entity and `ApiKeyRepository`
- [x] 5.6 Create `ApiKeyService`: create (returns plaintext once), rotate, deactivate, validate; resolve key to tenant + shelter (optional) + implicit role (COORDINATOR for shelter-scoped keys, COC_ADMIN for org-level keys where shelter_id is null)
- [x] 5.7 Create `ApiKeyAuthenticationFilter`: parse X-API-Key header, validate, resolve tenant/shelter/role, set SecurityContext
- [x] 5.8 Create `SecurityConfig`: configure filter chain with OAuth2 login → JWT filter → API key filter, role-based endpoint authorization
...
- [x] 5.16 Add integration test: verify role-based access (coordinator cannot access admin endpoints)
- [x] 5.17 Add integration test: verify DV RLS (user without dvAccess cannot see DV shelters via any query path)
- [x] 5.18 Add integration test: verify OAuth2 login links to pre-created account and rejects unknown emails
- [x] 5.19 Add integration test: verify JWT from OAuth2 login is identical in structure to password-based JWT

## 6. Shelter Profile

- [x] 6.1 Create `Shelter` entity with all fields from migration V4
- [x] 6.2 Create `ShelterConstraints` entity (embedded or separate table reference)
- [x] 6.3 Create `ShelterCapacity` entity for per-population-type capacity
- [x] 6.4 Create `PopulationType` enum: SINGLE_ADULT, FAMILY_WITH_CHILDREN, WOMEN_ONLY, VETERAN, YOUTH_18_24, YOUTH_UNDER_18, DV_SURVIVOR
- [x] 6.5 Create `ShelterRepository` with tenant-scoped queries and constraint filtering
- [x] 6.6 Create `ShelterService` with CRUD operations, coordinator assignment validation
- [x] 6.7 Create `ShelterController`: POST/GET/PUT `/api/v1/shelters`, GET `/api/v1/shelters/{id}` with pagination and constraint filters
- [x] 6.8 Create `ShelterHsdsMapper`: map FABT shelter to HSDS 3.0 JSON with `fabt:` extension namespace
- [x] 6.9 Add GET `/api/v1/shelters/{id}?format=hsds` endpoint for HSDS-compatible export
- [x] 6.10 Create coordinator assignment endpoints: POST/DELETE `/api/v1/shelters/{id}/coordinators`
- [x] 6.11 Add integration test: verify constraint filtering returns correct shelters
- [x] 6.12 Add integration test: verify coordinator can only update assigned shelters

## 7. Data Import

- [x] 7.1 Create `ShelterImportService` with validation pipeline: validate → deduplicate (name + address within tenant; duplicate = full replace of all fields from import source) → persist → report
- [x] 7.2 Create `HsdsImportAdapter`: parse HSDS 3.0 JSON, map to FABT shelter model, feed to ShelterImportService
- [x] 7.3 Create `TwoOneOneImportAdapter`: parse 211 CSV, fuzzy column header matching, preview mapping, feed to ShelterImportService
- [x] 7.4 Create `ImportController`: POST `/api/v1/import/hsds` (multipart file), POST `/api/v1/import/211` (multipart file), GET `/api/v1/import/211/preview` (column mapping preview)
- [x] 7.5 Create `ImportLog` entity and `ImportLogRepository` for audit trail
- [x] 7.6 Add integration test: HSDS import creates shelters and handles duplicates
- [x] 7.7 Add integration test: 211 CSV import with non-standard column names (fuzzy header matching, unmapped column warnings in report)
- [x] 7.8 Add integration test: verify population type enum validation returns all 7 valid types in 400 error response
