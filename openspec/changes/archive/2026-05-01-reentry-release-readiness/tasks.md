...
- [x] 11.2 **`frontend/src/i18n/es.json`** — Spanish translations for all 4 new keys + the privacy-note reconciliation. Spanish copy was warroom-output drafted (Casey/Keisha voice), not machine translation. **AI-SYNTHETIC-LINGUISTIC-REVIEW (Claude with web-citation grounding, NOT a native speaker) — see `docs/audits/2026-05-01-v0-55-1-spanish-review/synthetic-maria-pass.md`** (will resolve to `archive/2026-05-XX-v0-55-1-followup/audit/synthetic-maria-pass.md` once v0.55.1 archives). Pass landed in v0.55.1-D2 with 3 of 5 keys revised (clientDob register-consistency, notes anglicism disambiguation, eligibility-notes outreach pan-Latin neutrality). Real-native-reviewer pass remains a future option; the AI-synthetic pass with cited Spanish-language sources makes a future review faster.
- [x] 11.3 **`HoldDialog.tsx`** — privacy framing renders OUTSIDE the disclosure (always-visible `<p>` with `hold.clientAttributionPrivacyNote` above the toggle, per warroom Round 2). The `<details data-testid="hold-attribution-toggle">` itself ships **collapsed by default** so the no-attribution Enter path is unobstructed; per-input help text renders INSIDE the disclosure with `aria-describedby="<id>-help"` and a `<p>` carrying the i18n string. Confirm button still auto-focused — no-attribution Enter path unchanged.
...
- [x] 12.1 **Live-site claim audit** (script-driven if possible, manual grep otherwise) — fetch `https://findabed.org/` after the §2.1 fix lands locally + check that no platform-wide "ever" / "never client name" / "zero PII" claim remains unscoped. Repeat for `for-cities.html`, `for-coc-admins.html`, `for-funders.html`, `for-coordinators.html`.
- [x] 12.2 **Screenshot drift check** completed (§10) and documented.
- [x] 12.3 **Capture spec runs green** end-to-end via updated `capture.sh` (§7.3 + §8.4).
- [x] 12.4 **ZAP-baseline note** — document that `docs/security/zap-v0.55-baseline.md` is deferred to a follow-up slice (it's in proposal Out of Scope). Confirm the deferral is captured in v0.56 backlog.
- [x] 12.5 **Backend test suite** runs green (`mvn -B test -q` in code repo) — backend code is unchanged but verify the doc-only edits didn't break any test that grep's documentation files.
- [x] 12.6 **Frontend build + Playwright suite** runs green (`npm run build` + `npx playwright test --reporter=list` in code repo `e2e/playwright`).
- [x] 12.4.5 **Spanish translations non-identical to English** — for every new i18n key added in §11.1 / §11.2, assert the `es.json` value differs from the `en.json` value (catches "translation forgotten" silent failures).
- [x] 12.5.5 **Scheduled-invocation integration test** — `HoldAttributionIntegrationTest.scheduledEntryPoint_purgesAgedRow_endToEnd` (file: `backend/src/test/java/org/fabt/reservation/HoldAttributionIntegrationTest.java:738`) calls `referralTokenPurgeService.purgeExpiredHoldAttribution()` (no args, no outer `WithTenantContext`) against an aged row and asserts ciphertext is nulled. Closes Riley R-RR-1 by exercising the full `@Scheduled` wrapper path. 23/23 tests in the class GREEN.
- [x] 12.6.5 **axe-core run on `demo/reentry-story.html`** — run via Playwright; zero violations required (ADA Title II, Tomás TO-RR-2).
- [x] 12.7 **Legal language scan** runs clean (per `feedback_legal_claims_review` + `feedback_legal_scan_in_comments`) — grep all touched files for "compliant," "ensures," "guarantees," "equivalent to," "secure," "fully," "complete," "comprehensive," "certified," "audited" — replace with "designed to support" or specific factual statements. **Scan code comments + JavaDoc**, not just user-facing prose; `Reservation.java:22` JavaDoc currently says "plaintext is never persisted to disk" — verify or revise.
- [x] 12.7.5 **Banned-words automated grep** (per Keisha K-RR-1) — script-level grep across `demo/reentry-story.html` AND every doc touched in §2 / §3 / §4 / §5 / §9 for the banned-words list: ex-offender, returning citizen, felon, offender, justice-involved, individuals with criminal histories, formerly incarcerated individuals, reentering population. Zero matches required. Suggested: add as a CI guard script under `scripts/ci/` so future changes can't reintroduce.
- [x] 12.8 **Stakeholder-name scan** runs clean — grep all touched files for the names of real NC cities, real CoCs, real reentry programs, real hospitals. Zero matches.
...
- [ ] 15.6 **Demo flow walkthrough** as `outreach@blueridge.fabt.org` — exercise the reentry advanced filters, hold dialog with attribution, verify tooltips render, confirm the empty-state path: filter TRANSITIONAL + Buncombe + accepts-felonies (matches reentry-06 capture per §7.1) → 0 shelters found + the §8/§9 H4 empty-state banner ("No matching shelters. The 'accepts felonies' filter excludes shelters with no eligibility data on file.") renders with active-filter chips visible.
