...
- **Charts**: Recharts — React-native, lightweight, good for time-series and bar charts. Lazy-loaded via `React.lazy()` — the ~200KB bundle only loads when the admin opens the Analytics tab. Outreach workers on phones never download it.
- **Map**: react-leaflet with Leaflet — open-source, no API key required (OpenStreetMap tiles). DV shelters excluded from map per D4. Fallback: if map tiles fail to load (air-gapped deployment), show a table with city/zip grouping instead of a blank map.
...
### D18: HMIS DV small-cell suppression (compliance fix)

The existing `HmisTransformer.buildInventory()` aggregates all DV shelters into a single "DV Shelters (Aggregated)" record. However, if a CoC has only one DV shelter, the aggregate count IS the individual shelter's count — defeating the purpose of aggregation. This is a compliance defect flagged by Dr. Kenji Watanabe, aligned with CMS small-cell suppression guidance.

**Fix — dual threshold suppression:**
1. **Minimum distinct shelters**: suppress DV aggregate if `COUNT(DISTINCT dv_shelters) < 3`
2. **Minimum bed count**: suppress if `dvTotalBeds < 5` (existing check in analytics, missing in HMIS)
3. Both conditions must pass — either failing means suppression

**Applied to all DV aggregate outputs:**
- `HmisTransformer.buildInventory()` — suppress DV aggregate record entirely if thresholds not met
- `AnalyticsService.getDvSummary()` — add shelter count check alongside existing bed count check
- `HicPitExportService.generateHic()` — suppress DV row if thresholds not met
- `HicPitExportService.generatePit()` — suppress DV row if thresholds not met

**When suppressed:**
- HMIS push: DV aggregate record omitted entirely (no partial data)
- Analytics API: returns `{ "suppressed": true, "reason": "Insufficient DV shelters for safe aggregation" }`
- HIC/PIT CSV: DV row omitted

**Rationale:** CMS standard is minimum cell size 11, but that applies to client-level data. FABT reports only project-level bed counts (no PII), so the re-identification risk is shelter identity, not client identity. A minimum of 3 distinct shelters prevents the "aggregate = individual" problem while being practical for small CoCs. The bed count threshold of 5 provides an additional safety margin.
