...
- **Webhook subscription management**: delete (soft delete via CANCELLED status), pause/resume via existing `status` field (PAUSED value — no new boolean column), test event with timeout (10s connect, 30s read), delivery log (last 20 per subscription, response body truncated to 1KB). Auto-disable: 5 consecutive failures → DEACTIVATED status. State machine: ACTIVE↔PAUSED, ACTIVE→FAILING→DEACTIVATED→ACTIVE.
