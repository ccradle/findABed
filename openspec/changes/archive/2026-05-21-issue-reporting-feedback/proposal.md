...
- Add a **"Report a Problem" link** in the authenticated app footer, visible across authenticated pages, linking to the GitHub `report-a-problem` issue template with pre-filled context (version only — see design.md §6 for the privacy-driven exclusion list)
...
- **Small backend delta**: `ContactInfoController.buildResponseBody` extended to surface an existing-evaluated boolean (`tenant.dvPolicyEnabled`) on the public `GET /api/v1/public/contact-info` response. No new endpoint, no new migration, no new dependency — just adding one field to a response the controller already builds. Frontend `useContactInfo()` hook shape gains the boolean.
