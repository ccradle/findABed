...
- **NEW `deploy/rehearsal-prod-overlay.yml`** — committed compose overlay that mirrors the operator-side `~/fabt-secrets/docker-compose.prod*.yml` chain WITHOUT real secrets, so the rehearsal exercises the merge layout that prod actually runs. Includes `axllent/mailpit` as the SMTP stub (Mailpit is the maintained successor to MailHog, which was abandoned in 2020).
...
- `demo-211-import-edit` Playwright test: fix strict mode violation at line 192 — locator `'[role="alert"], [data-testid="error-message"]'` matches both the import error div AND the SSE `critical-notification-banner` when a prior test leaves a notification visible. Fix narrows to `'[role="alert"]:not([data-testid="critical-notification-banner"])'` so the selector is unambiguous regardless of SSE notification state. (Chromium-only failure observed in full-suite run; nginx project passed because the notification had expired by the time it ran.)
...
- **Modified files:** `deploy/release-gate-pins.txt` (one new pin), `scripts/ci/verify-release-gate-pins.sh` (one new check if the pin format demands it), `docs/FOR-DEVELOPERS.md` (one new "Pre-tag rehearsal" subsection), `e2e/playwright/tests/demo-211-import-edit.spec.ts` (line 192 — locator narrowed)
